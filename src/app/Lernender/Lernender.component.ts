...
export class LernenderComponent implements OnInit {
  ...
  // This Function create a PDF with the details about all Lernende
  pdfExport(): void {
    console.log("PDF Export Triggered");

    // Create a new jsPDF const
    const doc = new jsPDF({
    ...
    // Set font size and write the title
    doc.setFontSize(15);
    ...
    for (let i = 0; i < this.allParticipants.length; i++) {
      ...
      const y = (i % 7) * 10 + 20; // reset y 
      
      // Set font size
      doc.setFontSize(15);

      // Get Lernender attributes
      let lernenderVorname = this.allParticipants[i].vorname;
      let lernenderNachname = this.allParticipants[i].nachname;
      let lernenderGeburtsdatum = this.allParticipants[i].geburtsdatum;
      let lernenderLehrbginn = this.allParticipants[i].lehrbeginn;
      let lernenderLehrende = this.allParticipants[i].lehrende;

      // Write Lernender attributes into PDF
      doc.text(lernenderVorname, 10, y + 10);
      doc.text(lernenderNachname, 35, y + 10);
      doc.text(lernenderGeburtsdatum, 70, y + 10);
      doc.text(lernenderLehrbginn, 100, y + 10);
      doc.text(lernenderLehrende, 120, y + 10);
    }
    // Save the document
